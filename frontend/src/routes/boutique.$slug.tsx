...
import { fetchProductBySlug, fetchSimilarProducts } from "@/lib/api/products";
...
export const Route = createFileRoute("/boutique/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        { title: p ? `${p.nom} — OBB Store` : "Produit — OBB Store" },
        { name: "description", content: p?.description.slice(0, 160) ?? "Produit OBB Store" },
        { property: "og:title", content: p ? `${p.nom} — OBB Store` : "OBB Store" },
        { property: "og:description", content: p?.description.slice(0, 160) ?? "OBB Store" },
        ...(p?.imagePrincipale ? [{ property: "og:image", content: p.imagePrincipale }] : []),
      ],
    };
  },
  notFoundComponent: () => (
...
function ProductPage() {
  ...
  const { data: related = [] } = useQuery({
    queryKey: ["products", "similar", product.slug],
    queryFn: () => fetchSimilarProducts(product.slug, 3),
  });
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16">
        ...
        <div className="lg:py-8 space-y-6 sm:space-y-8">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-display mb-4">{product.nom}</h1>
            <div className="flex items-center gap-3 sm:gap-4">
              <p className="font-mono text-xl sm:text-2xl font-bold text-accent">{formatPrice(product.prix)}</p>
              {product.ancienPrix && product.ancienPrix > product.prix && (
                <>
                  <p className="font-mono text-base sm:text-lg text-muted line-through">{formatPrice(product.ancienPrix)}</p>
