...
function BoutiquePage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["products", cat, query, sortBy, priceRange],
    queryFn: () =>
      fetchProducts({
        cat,
    ...
    <div className="min-h-screen bg-[#F5F8FC] text-[#0A0A0A]">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-16">
        <div className="font-mono text-[10px] uppercase tracking-tighter text-[#0A0A0A]/60 mb-6 sm:mb-8">
          {isLoading ? "…" : `${String(products.length).padStart(2, "0")} / ${String(totalProducts).padStart(2, "0")} produits`}
        </div>
        {isLoading ? (
          <div className="py-16 sm:py-32 text-center text-[#0A0A0A]/60">Chargement…</div>
        ) : products.length === 0 ? (
          <div className="py-16 sm:py-32 text-center text-[#0A0A0A]/60">Aucun produit ne correspond.</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-x-6 sm:gap-y-10 lg:gap-x-8 lg:gap-y-12">
            {products.map((p) => (
