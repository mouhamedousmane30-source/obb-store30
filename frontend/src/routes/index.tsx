...
import { fetchPopularProducts, fetchNewProducts } from "@/lib/api/products";
...
function HomePage() {
  const { data: popularData } = useQuery({
    ...
    queryFn: () => fetchPopularProducts(6),
    staleTime: 5 * 60 * 1000,
  });
  const { data: newData } = useQuery({
    queryKey: ["products", "new"],
    queryFn: () => fetchNewProducts(4),
    staleTime: 5 * 60 * 1000,
  });
  ...
  const popular = popularData?.length
    ? popularData
    : PRODUCTS.filter((p) => p.isPopular).slice(0, 6);
  const newArrivals = newData?.length
    ? newData
  ...
  const categories = categoriesData?.length
    ? categoriesData.map((c) => ({
        ...
        hoverImage: c.hoverImage || CATEGORIES.find((x) => x.slug === c.slug)?.hoverImage,
        productsCount: c.productsCount,
      }))
    : CATEGORIES.map((c) => ({
        ...c,
        productsCount: PRODUCTS.filter((p) => p.category === c.slug).length,
      }));
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
          {categories.map((cat, index) => (
            <Link
              key={cat.slug}
              to="/boutique"
              search={{ cat: cat.slug as "maillots" | "tshirts" | "chaussures" | "parfums" }}
              ...
              <div className="absolute inset-0 flex flex-col items-end justify-between p-4 sm:p-6">
                <div className="bg-[#1E40AF]/40 backdrop-blur-md rounded-full px-3 sm:px-4 py-1.5 sm:py-2 border border-[#1E40AF]/30">
                  <span className="text-white text-[10px] uppercase tracking-widest font-semibold">
                    {(cat as { productsCount?: number }).productsCount ?? PRODUCTS.filter((p) => p.category === cat.slug).length} articles
                  </span>
                </div>
